...
function renderDashboard() {
  ...
  const incomplete = records.filter(r => r.info_incomplete);
  const incompleteN = incomplete.length;
  ...
  document.getElementById('statCards').innerHTML = `
    ...
    ${incompleteN ? `<div class="stat-card incomplete"><div class="num">${incompleteN}</div><div class="label">⚠ 信息缺失待补全</div></div>` : ''}
  `;
  // 2026-09-10: 信息缺失提醒横幅（游客端已隐藏，管理员请尽快补全）
  if (incompleteN) {
    const lines = incomplete.slice(0, 12).map(r => `· ${r.flight||''}/${r.flight_return||''} ${r.dep}→${r.arr} ${r.dep_date} ${r.supplier||''}（缺 ${(r.info_incomplete_fields||[]).join('/')}）`).join('<br>');
    let box = document.getElementById('incompleteBanner');
    if (!box) {
      box = document.createElement('div');
      box.id = 'incompleteBanner';
      box.className = 'incomplete-banner';
      const sc = document.getElementById('statCards');
      if (sc && sc.parentNode) sc.parentNode.insertBefore(box, sc.nextSibling);
    }
    if (box) box.innerHTML = `<b>⚠ 信息缺失待补全（${incompleteN} 条，游客端已隐藏，请尽快补全）：</b><br>${lines}`;
  }
